...
module.exports.bookingRequestSchema = Joi.object({
  booking: Joi.object({
    ...
    tripType: Joi.string().valid('local', 'outstation', 'airport-pickup', 'railway-pickup', 'wedding', 'family-function', 'temple-visit', 'tourism', 'corporate', 'other').optional().default('local'),
    specialNote: Joi.string().trim().allow("", null).optional(),
    specialInstructions: Joi.string().trim().allow("", null).optional(),
    distanceKm: Joi.number().min(0).optional().allow(""),
    estimatedDistance: Joi.number().min(0).optional().allow(""),
    estimatedDuration: Joi.string().allow("", null).optional(),
    estimatedFare: Joi.number().min(0).optional().allow(""),
    basePrice: Joi.number().min(0).optional().allow(""),
    pricePerKM: Joi.number().min(0).optional().allow(""),
    ...
    pickupLat: Joi.number().optional().allow(""),
    pickupLng: Joi.number().optional().allow(""),
    destLat: Joi.number().optional().allow(""),
    destLng: Joi.number().optional().allow("")
  }).required()
});
